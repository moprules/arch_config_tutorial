...
## 2.1 Особенности установки в dualboot (windows + linux)
Если linux ставится в параллель с Windows, то __обязательно диск должен быть размечен заранее__.
Если изменять размер раздела, на котором уже установлен Windows, только во время установки archlinux, то сотрётся загрузчик Windows.
Лучше всего действовать в таком порядке:  
_1)_ Загрузится в livecd и выделить пространство для linux и Windows.  
_2)_ Установить Windows.  
_3)_ Установить linux.

## 2.2 Установка раскладки клавиатуры
...
## 2.3 Соединение с интернетом по Wi-fi
...
## 2.4 Синхронизация системных часов
```console
timedatectl set-ntp true
```

## 2.5 Разбиение дисков на разделы
...
## 4.11 Генерация нового ssh ключа
```console
ssh-keygen -t ed25519 -C "your_email@example.com"
```
```console
ssh-keygen -t ed25519 -C "rav-navini-gego-cutropal@yandex.ru.com"
```

## 4.12 Авторизации на сервере по ключу
Первый перенос ключа на сервер в `authorized_keys`
```console
ssh-copy-id -i /path/to/your_server_key.pub user@server_domain_or_ip
```

После можно авторизовываться с локального ПК на сервер так:
```console
ssh -i /path/to/your_server_key user@server_domain_or_ip
```

Или прописать отдельные строки в `~/.ssh/config`
```console
Host my_server
  HostName server_domain.ru
  User root
  Preferredauthentications publickey
  IdentityFile /path/to/your_server_key
```

Дальше можно авторизовываться простой коммандой
```console
ssh my_server
```
